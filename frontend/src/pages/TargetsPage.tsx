...
const STATUS_META: Record<TargetStatus, { label: string; variant: 'info' | 'warning' | 'default' }> = {
  PREPARING:    { label: '准备中', variant: 'info' },
  INTERVIEWING: { label: '面试中', variant: 'warning' },
  CLOSED:       { label: '已结束', variant: 'default' },
}
    ...
    <div className="p-6 space-y-6">
      ...
      {targets.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {targets.map(t => (
            <Card key={t.id} hoverable onClick={() => navigate(`/targets/${t.id}`)} className="group">
              <CardBody className="flex items-center gap-4">
                <ReadinessRing value={t.readiness} size={84} stroke={7} />
